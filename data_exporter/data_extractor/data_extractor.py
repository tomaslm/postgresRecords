...
class DataExtractor:
    ...
    def get_header_with_labels(self, export_type):
        ...
        return self.db.query_file(file_path, fetchall=True).as_dict(ordered=True)
    ...
    def _export_to_file(self, file, export_type, configs, filters):
        ...
        def format_values(row):
            return [
                self.format_value_by_type(row.get(prop), configs)
...
if __name__ == "__main__":
    ...
    configs = {timezone: timezone_sao_paulo}

    filters = {
        "ids": (1, 2, 3, 4, 5, 6, 7),
        "date_min": datetime.datetime(2018, 12, 6, 00, 00).astimezone(
            timezone_sao_paulo
        ),
        "date_max": datetime.datetime(2019, 1, 1, 00, 00).astimezone(
            timezone_sao_paulo
        ),
    }
    ...
    with open("data_extractor_test.csv", "w+") as test_file:
        data_extractor._export_to_file(
            file=test_file, export_type="order", configs=configs, filters=filters
        )
